...
>
> **9/13 跨天账本 + fetch 走 9222（重构）**：① **跨天去重账本**（治「同一事件连续多天整条重复成稿」）——生产账本在 `~/.ai-daily/published-ledger.json`（HOME 而非 iCloud：launchd TCC 读不了 Mobile Documents，8/31 P4 实证；HOME 路径有 linuxdo-prefetch.json 先例），记录端 = `finalize.mjs`（写完 4 产物自动追加 confirmed 条目：URL+指纹 token+day+major；**非生产 outDir 不记账 → `LEDGER-SKIP`**，烟测 /tmp 天然隔离；显式 `--ledger` 可覆盖）。编排器（手动与 headless 一致）在调 Workflow 前 **Read 该账本文件**：存在 → 把 JSON 数组作为 `args.reportedLedger` 传入；不存在（首跑）→ 不传该参数、不要报错。realm 内消费三道：近 3 天已报道 URL/指纹**硬过滤**（fetch 配额前，meta `reported_deduped:N`）、已报道**种子退役**（`SEED-LEDGER` 日志，报过一次即退役，不再 21 天逐日注入）、近 3 天已报道**名单进 report 输入**（软网，prompt 4.8：同事件换 URL 一律不写正文，实质新增只写增量并标「前情提要」）。无账本 → fail-open + degraded 旗标 `ledger_unavailable`。meta 增 `reported_deduped` / `major_dup_skipped` / `ledger_entries`。② **fetch 走 9222**（治「除 linux.do 外全部抓取不走登录态浏览器」）——`args.webFetchViaCdp: true`（headless 显式传；手动默认 false 维持纯 WebFetch）时，fetch 代理 Step1 先经 Bash 跑宿主 CLI `node scripts/ai-daily/cdp-fetch.mjs '<url>'`（9222 临时标签读正文，`{ok:true,url,chars,text}` JSON 到 stdout；失败非零退出+stderr 诊断），失败回落 WebFetch 文本抓取。**索引页治理**（两种模式都生效）：`found_via=static-fallback` 的来源是栏目/索引页——只做发现入口，从索引页选窗口内真实文章 URL 逐条抓正文，claim 带可选 `sourceUrl`（EXTRACT_SCHEMA 新增字段）= 真实文章 URL，template 注入点按 http(s) 合法性优先取之——引用/verify/角标全部落到文章页。`cdp-fetch.mjs` 是**宿主 Node CLI 不 inline 进产物**（与 linuxdo-prefetch 同款；CDP 协议层抽在 `cdp-core.mjs`，build MODULES 含之供 linuxdo.mjs 复用）。纪律不变：只复用已运行 9222、只关自开临时标签、绝不关浏览器本体；并发信号量上限 3（`~/.ai-daily/cdp-locks/lock-0.lock`…`lock-2.lock` 固定槽 `wx` 抢位，锁等待 = 一次完整读页 35s（open+poll+5s，9/19 C2）、持锁每 10s 心跳续期 mtime、>60s 无心跳才陈旧回收（9/19 C3，活锁不再被误回收））。major-out 注入 `_majorDupCheck` 先查 `REPORTED_LEDGER`（跨天命中返回 `ledger`，不得只互斥本窗）；`storyMatch` 对带连字符、**含字母**、≥8 的 ASCII 产品名（如 V4-Flash-Vision-Exp）精确命中账本 token，避免短 name 打不满 SHARE_MIN；**ISO 日期（`2026-09-13`）不得当强实体**（日报/linux.do 标题几乎每天都带日期）。

> **9/19 内容质量三件套（核查真实性 / 聚类误合 / 状态烘焙）**：① **外部抽查票**——forum/blog 来源的存活 claim 加 1 张**允许 WebSearch** 的独立佐证票（`externalVerifyPrompt`；内部 2+1 票禁外部搜索只做文本自洽，「引语整齐但不实」的论坛转述恰是盲区，9/17 实证网信办条目唯一来源是论坛转帖）：否决 → 翻转回 killed；佐证 → `externalCheck=corroborated`；票不可用 → `externalCheck=unavailable` 且 Status 烘焙为「未核查」（degraded 旗标 `external_check_unavailable:N`，meta `external_check{targets,corroborated,refuted,unavailable}`）。② **Status 烘焙**——reportBody 素材行由编排层预烘 `Status:`（投票比 + 外部抽查结果），report prompt §4 只许照抄、禁止自行推导（旧版模型手抄投票比写错字面量即徽标丢失）。③ **聚类误合治理**——CJK 阈值 3→4、四字通用术语内部 bigram 全停用（人工智能/机器学习/数据中心等单个四字词即可凑满旧阈值 3 是误合根因）、簇大小上限 4（桥接 token 传递闭包不再串 5+ 条）、`mergeCluster` status 取「最已核查」（首条 `[窗口外·重大]` 不再把已核查项拖成未核查）。ledger 指纹改 ASCII/CJK **分桶各留 32**（旧版整体截 64、ASCII 在前，长英文 snippet 的中文 bigram 全被截掉 → 中文重复漏过硬过滤）。
