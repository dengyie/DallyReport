...
>
> **9/13 跨天账本 + fetch 走 9222（重构）**：① **跨天去重账本**（治「同一事件连续多天整条重复成稿」）——生产账本在 `~/.ai-daily/published-ledger.json`（HOME 而非 iCloud：launchd TCC 读不了 Mobile Documents，8/31 P4 实证；HOME 路径有 linuxdo-prefetch.json 先例），记录端 = `finalize.mjs`（写完 4 产物自动追加 confirmed 条目：URL+指纹 token+day+major；**非生产 outDir 不记账 → `LEDGER-SKIP`**，烟测 /tmp 天然隔离；显式 `--ledger` 可覆盖）。编排器（手动与 headless 一致）在调 Workflow 前 **必须 Read 该账本文件**：存在 → 把 JSON 数组作为 `args.reportedLedger` 传入（禁止因 resume/漏读而省略）；不存在（首跑）→ 不传该参数、不要报错。realm 内消费三道：近 3 天已报道 URL/指纹**硬过滤**（fetch 配额前，meta `reported_deduped:N`）、已报道**种子退役**（`SEED-LEDGER` 日志，报过一次即退役，不再 21 天逐日注入）、近 3 天已报道**名单进 report 输入**（软网，prompt 4.8：同事件换 URL 一律不写正文，实质新增只写增量并标「前情提要」）。无账本 → fail-open + degraded 旗标 `ledger_unavailable`。meta 增 `reported_deduped` / `major_dup_skipped` / `ledger_entries`。② **fetch 走 9222**（治「除 linux.do 外全部抓取不走登录态浏览器」）——`args.webFetchViaCdp: true`（headless 显式传；手动默认 false 维持纯 WebFetch）时，fetch 代理 Step1 先经 Bash 跑宿主 CLI `node scripts/ai-daily/cdp-fetch.mjs '<url>'`（9222 临时标签读正文，`{ok:true,url,chars,text}` JSON 到 stdout；失败非零退出+stderr 诊断），失败回落 WebFetch 文本抓取。**索引页治理**（两种模式都生效）：`found_via=static-fallback` 的来源是栏目/索引页——只做发现入口，从索引页选窗口内真实文章 URL 逐条抓正文，claim 带可选 `sourceUrl`（EXTRACT_SCHEMA 新增字段）= 真实文章 URL。**9/21 夜（同夜 review 改名）**：注入走 `bindExtractedClaims`——索引页缺合法 http(s) 文章 `sourceUrl` 的 claim **丢弃并计入** `index_claim_dropped:N`（计数字段 `indexClaimDropped`；日志 `INDEX-CLAIM-DROPPED`。已丢弃、未进引用，不得再叫 `index_page_citation`，也不得把栏目 URL 如 `https://x.ai/news` 注入引用/verify/角标）；文章页缺可选 `sourceUrl` 回落 `src.url`、**不计**旗标。窗口外参考走 `foldWindowMisses`（CJK bigram 共享 ≥4，词法名 `wm*`）折叠中文近重复；**列表内部**拉丁 token 必须共享 ≥2（`WM_ASCII_MIN_INTERNAL`，只共享一个厂商标记的两条不同事件不得互折），与正文/major-out 互折仍是共享 1 个拉丁 token 即去掉。`cdp-fetch.mjs` 是**宿主 Node CLI 不 inline 进产物**（与 linuxdo-prefetch 同款；CDP 协议层抽在 `cdp-core.mjs`，build MODULES 含之供 linuxdo.mjs 复用）。纪律不变：只复用已运行 9222、只关自开临时标签、绝不关浏览器本体；并发信号量上限 3（`~/.ai-daily/cdp-locks/lock-0.lock`…`lock-2.lock` 固定槽 `wx` 抢位，锁等待 = 一次完整读页 35s（open+poll+5s，9/19 C2）、持锁每 10s 心跳续期 mtime、>60s 无心跳才陈旧回收（9/19 C3，活锁不再被误回收））。major-out 注入 `_majorDupCheck` 先查 `REPORTED_LEDGER`（跨天命中返回 `ledger`，不得只互斥本窗）；`storyMatch` 对带连字符、**含字母**、≥8 的 ASCII 产品名（如 V4-Flash-Vision-Exp）精确命中账本 token，避免短 name 打不满 SHARE_MIN；**ISO 日期（`2026-09-13`）不得当强实体**（日报/linux.do 标题几乎每天都带日期）。
