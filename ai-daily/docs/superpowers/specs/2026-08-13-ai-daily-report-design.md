...
> - **回归守卫**：`test/linuxdo.test.mjs` 新增「WS 路径对每个开的标签都关」——mock CDP fetch（`/json/new`→`/json/close`）+ mock WebSocket（构造即 onopen、send 即回 body），断言 `closed.length === opened.length`。修复前该测试红（closed 0 vs opened 8、未关）、修复后绿。
> - **8/23 复核 fix（agent 复核收敛关闭点 + 补早退守卫）**：独立复核 agent 确认主修复方向正确（成功流 + HTTP 降级流 1:1 关闭、无双关、只关自己开的 target），但指出**统一关闭点只在函数末尾一行、非 `finally`**——WS open reject、open-tab 非 2xx 抛错等中途 throw 路径仍漏关标签（实测开 1 关 0）。修复：`readBodyText` 用 `try { ... } finally { await closeTab(host, targetId) }` 收敛唯一关闭点，open-tab 非 2xx 提前 throw（target 未建立、无标签可关），WS onerror 触发时先 `ws.close()` 再 reject；追加「WS onerror 早退后仍关掉已开的标签」回归用例，Red/Green 自证（修复前红、修复后绿）。复核判定 spec ✅ / quality ⚠️，修复该条 Important 后收线。**128 pass / 0 fail**（上一轮修复后基线 127 + 本轮新增 onerror 守卫 1；前文"127 pass（123+1）"数字自洽勘误）。
